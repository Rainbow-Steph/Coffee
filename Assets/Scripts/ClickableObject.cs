...
private Material originalMaterial;
    private Material[] originalMaterials;  // Store original materials array
  ...
  originalMaterials = objectRenderer.sharedMaterials;  // Store all materials
 }
        else
 {
...
Debug.Log($"ClickableObject: {gameObject.name} - Outline material applied");
 ...
 // Handle OutlineOverlay mode
        else if (highlightMode == HighlightMode.OutlineOverlay)
        {
            if (objectRenderer != null && outlineMaterial != null)
     {
              // Create materials array with original + outline
Material[] materials = new Material[originalMaterials.Length + 1];
     ...
     if (showDebugInfo)
        {
     Debug.Log($"ClickableObject: {gameObject.name} - Outline overlay applied ({materials.Length} materials)");
           }
            }
        }
    }
 ...
 if (showDebugInfo)
      {
     Debug.Log($"ClickableObject: {gameObject.name} - Original materials restored ({originalMaterials.Length} materials)");
     }
     }
        }
    }
...
Color emissionColor = hoverColor * hoverEmissionIntensity;
    ...
    /// <summary>
    /// Public method to manually set outline state (for OutlineMaterial/OutlineOverlay mode)
    /// </summary>
...
// Clean up any extra material instances (OutlineMaterial/OutlineOverlay mode)
        if ((highlightMode == HighlightMode.OutlineMaterial || highlightMode == HighlightMode.OutlineOverlay) && objectRenderer != null)
        {
   ...
   {
 foreach (Material origMat in originalMaterials)
        {
      if (mat == origMat)
           {
           isOriginal = true;
